...
const STATUSES=["En cours","En attente","Terminé","Bloqué","Annulé"];
...
const SC={"En cours":{bg:"#E6F1FB",tx:"#0C447C"},"En attente":{bg:"#FAEEDA",tx:"#633806"},"Terminé":{bg:"#EAF3DE",tx:"#27500A"},"Bloqué":{bg:"#FCEBEB",tx:"#791F1F"},"Annulé":{bg:"#ECECEC",tx:"#555555"}};
...
const STATUS_ORDER={"En cours":0,"Bloqué":1,"En attente":2,"Terminé":3,"Annulé":4};
...
function isOD(d,s){return s!=="Terminé"&&s!=="Annulé"&&!!d&&d<TODAY;}
function pgCol(v){return v>=70?"#639922":v>=40?"#BA7517":"#378ADD";}
function calcProgress(projectId, tasks){
  const pt=tasks.filter(t=>t.project_id===projectId&&t.status!=="Annulé");
...
function ReunionModal({onClose}){
  ...
  const refreshLocal=useCallback(async()=>{
    ...
    setProjects(Array.isArray(p)?p.filter(x=>!x.archived):[]);
...
export default function App(){
  ...
  const [showArchivedP,setShowArchivedP]=useState(false); const [showArchivedT,setShowArchivedT]=useState(false); const [fOverdueOnly,setFOverdueOnly]=useState(false);
  ...
  const EP={name:"",status:"En cours",priority:"Moyenne",deadline:"",progress:0,pilot:pilots[0]?.name||"",pilot2:"",site:sites[0]||SITES[0],description:"",notes:"",weight:0,created_at:TODAY,archived:false};
  const ET={project_id:null,name:"",status:"En attente",priority:"Moyenne",pilot:pilots[0]?.name||"",pilot2:"",site:sites[0]||SITES[0],deadline:"",notes:"",weight:0,created_at:TODAY,completion_date:"",archived:false};
  async function saveP(f){
    const{id,...d}=f;
    d.progress=calcProgress(f.id,tasks.filter(x=>!x.archived));
  ...
  async function saveT(f){
    ...
    if(pid){
      ...
      const prog=calcProgress(pid,(Array.isArray(updTasks)?updTasks:[]).filter(x=>!x.archived));
  ...
  async function delT(id){
    ...
    if(t&&t.project_id){const updTasks=tasks.filter(x=>x.id!==id&&!x.archived);const prog=calcProgress(t.project_id,updTasks);await sbUpd("projects",t.project_id,{progress:prog});}
    fetchAll();
  }
  async function archiveP(id,val){await sbUpd("projects",id,{archived:val});fetchAll();}
  async function archiveT(id,val){await sbUpd("tasks",id,{archived:val});fetchAll();}
  async function cancelP(id){
    const reason=window.prompt("Pourquoi ce projet est-il annulé ?","");
    if(reason===null)return;
    await sbUpd("projects",id,{status:"Annulé",cancel_reason:reason});
    fetchAll();
  }
  async function cancelT(id){
    const reason=window.prompt("Pourquoi cette tâche est-elle annulée ?","");
    if(reason===null)return;
    await sbUpd("tasks",id,{status:"Annulé",cancel_reason:reason});
    fetchAll();
  }
  ...
  const fp=applySort((showArchivedP?projects.filter(p=>p.archived):visibleProjects).filter(p=>(!fSt||p.status===fSt)&&(!fPil||p.pilot===fPil)&&(!fSite||p.site===fSite)),sortKey,sortDir);
  const ft=applySort((showArchivedT?tasks.filter(t=>t.archived):visibleTasks).filter(t=>(!fTPrj||t.project_id===Number(fTPrj))&&(!fTSt||t.status===fTSt)&&(!fPil||(t.pilot===fPil||t.pilot2===fPil))&&(!fTSite||t.site===fTSite)&&(!fOverdueOnly||isOD(t.deadline,t.status))),sortKey,sortDir);
  const activeN=visibleProjects.filter(p=>p.status!=="Terminé"&&p.status!=="Annulé").length;
  const odN=visibleTasks.filter(t=>isOD(t.deadline,t.status)).length;
  const doneN=visibleTasks.filter(t=>t.status==="Terminé").length;
  const avgN=visibleProjects.length?Math.round(visibleProjects.reduce((s,p)=>s+calcProgress(p.id,visibleTasks),0)/visibleProjects.length):0;
  // ─── buildReport ────────────────────────────────────────────
  function buildReport(){
    const projects=visibleProjects, tasks=visibleTasks; // exclure les éléments archivés du bilan
    const now=new Date();
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card" style="border-top:3px solid #D85A30">
        ...
        <div class="kpi-value" style="color:#D85A30">${tasks.filter(t=>t.status!=="Terminé"&&t.status!=="Annulé").length}</div>
      </div>
  ...
  return(
    <div style={{padding:"1rem 0",fontFamily:"Arial,sans-serif",color:"#111"}}>
      {pModal&&<Modal title={pModal.mode==="edit"?"Modifier le projet":"Nouveau projet"} onClose={()=>setPModal(null)}><ProjForm data={pModal.data} pilots={pilots} tasks={visibleTasks} onSave={saveP} onClose={()=>setPModal(null)}/></Modal>}
      {tModal&&<Modal title={tModal.mode==="edit"?"Modifier la tâche":"Nouvelle tâche"} onClose={()=>setTModal(null)}><TaskForm data={tModal.data} projects={visibleProjects} pilots={pilots} onSave={saveT} onClose={()=>setTModal(null)}/></Modal>}
      ...
      {gantt&&<Modal title="Gantt — cliquer ▶ pour voir les tâches" onClose={()=>setGantt(false)} wide><GanttView projects={visibleProjects} tasks={visibleTasks} pilots={pilots}/><div style={{textAlign:"right",marginTop:12}}><button style={ss.btnS} onClick={()=>setGantt(false)}>Fermer</button></div></Modal>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
        {[
          {l:"Projets actifs",v:activeN},
          {l:"Tâches en retard",v:odN,d:odN>0,click:()=>{setView("tasks");setFOverdueOnly(true);setFTSt("");setFTPrj("");setFTSite("");setShowArchivedT(false);}},
          {l:"Tâches terminées",v:doneN,click:()=>{setView("tasks");setFTSt("Terminé");setFOverdueOnly(false);setFTPrj("");setFTSite("");setShowArchivedT(false);}},
          {l:"Avancement moyen",v:avgN+"%"}
        ].map(k=>(
          <div key={k.l} onClick={k.click} style={{background:"#f4f4f4",borderRadius:8,padding:"9px 11px",cursor:k.click?"pointer":"default"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid #ddd",marginBottom:12}}>
        {[["projects","Projets ("+visibleProjects.length+")"],["tasks","Tâches ("+visibleTasks.length+")"],["stats","Statistiques pilotes"],["todo","To-do list ✓ ("+todos.filter(t=>!t.done).length+"/"+todos.length+")"]].map(([v,l])=>(
      ...
      {view==="stats"&&<StatsView projects={visibleProjects} tasks={visibleTasks} pilots={pilots} todos={todos} dateFrom={reportDateFrom} setDateFrom={setReportDateFrom} dateTo={reportDateTo} setDateTo={setReportDateTo}/>}
      {view!=="stats"&&view!=="todo"&&<>
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {view==="projects"&&<button onClick={()=>setShowArchivedP(a=>!a)} style={{...ss.btnS,fontSize:12,background:showArchivedP?"#e8f0fb":"#f0f0f0",color:showArchivedP?"#1a6bbf":"#333",fontWeight:showArchivedP?700:400}}>{showArchivedP?"← Projets actifs":"📦 Afficher les projets archivés"}</button>}
          {view==="tasks"&&<button onClick={()=>setShowArchivedT(a=>!a)} style={{...ss.btnS,fontSize:12,background:showArchivedT?"#e8f0fb":"#f0f0f0",color:showArchivedT?"#1a6bbf":"#333",fontWeight:showArchivedT?700:400}}>{showArchivedT?"← Tâches actives":"📦 Afficher les tâches archivées"}</button>}
          {view==="tasks"&&fOverdueOnly&&<button onClick={()=>setFOverdueOnly(false)} style={{...ss.btnD,fontSize:12}}>✕ Filtre « en retard »</button>}
          ...
          return(
            <div key={p.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"11px 13px",marginBottom:9}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:8,flexWrap:"wrap",marginBottom:5}}>
                ...
                {p.archived&&<Badge label="Archivé" c={{bg:"#eee",tx:"#666"}}/>}
                <Badge label={p.status} c={SC[p.status]||{bg:"#eee",tx:"#333"}}/>
              ...
              {p.status==="Annulé"&&p.cancel_reason&&<div style={{fontSize:11,color:"#7a1f1f",background:"#faeded",border:"1px solid #e3b8b8",borderRadius:6,padding:"5px 9px",marginBottom:6,whiteSpace:"pre-wrap"}}>🚫 Annulé : {p.cancel_reason}</div>}
              ...
              <div style={{display:"flex",gap:6}}>
                ...
                <button style={{...ss.btnS,...(readOnly?{opacity:0.35,cursor:'not-allowed',pointerEvents:'none'}:{})}} onClick={()=>!readOnly&&archiveP(p.id,!p.archived)}>{p.archived?"Désarchiver":"Archiver"}</button>
                {p.status!=="Annulé"&&<button style={{...ss.btnD,...(readOnly?{opacity:0.35,cursor:'not-allowed',pointerEvents:'none'}:{})}} onClick={()=>!readOnly&&cancelP(p.id)}>Annuler</button>}
          ...
          return(
            <div key={t.id} style={{background:"#fff",border:"1px solid "+(od?"#f09595":"#e0e0e0"),borderRadius:8,padding:"9px 12px",marginBottom:7}}>
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:t.notes?6:0}}>
                ...
                {t.archived&&<Badge label="Archivé" c={{bg:"#eee",tx:"#666"}}/>}
                <Badge label={t.status} c={SC[t.status]||{bg:"#eee",tx:"#333"}}/>
                ...
                <div style={{display:"flex",gap:6,marginLeft:"auto"}}>
                  ...
                  <button style={{...ss.btnS,...(readOnly?{opacity:0.35,cursor:'not-allowed',pointerEvents:'none'}:{})}} onClick={()=>!readOnly&&archiveT(t.id,!t.archived)}>{t.archived?"Désarchiver":"Archiver"}</button>
                  {t.status!=="Annulé"&&<button style={{...ss.btnD,...(readOnly?{opacity:0.35,cursor:'not-allowed',pointerEvents:'none'}:{})}} onClick={()=>!readOnly&&cancelT(t.id)}>Annuler</button>}
                  <button style={{...ss.btnD,...(readOnly?{opacity:0.35,cursor:'not-allowed',pointerEvents:'none'}:{})}} onClick={()=>!readOnly&&delT(t.id)}>Suppr.</button>
                </div>
              </div>
              {t.status==="Annulé"&&t.cancel_reason&&<div style={{fontSize:11,color:"#7a1f1f",background:"#faeded",border:"1px solid #e3b8b8",borderRadius:5,padding:"4px 9px",marginBottom:t.notes?4:0,whiteSpace:"pre-wrap"}}>🚫 Annulé : {t.cancel_reason}</div>}
