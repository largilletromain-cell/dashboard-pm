...
async function sbGet(table, select="*") {
  const r = await fetch(SUPA_URL+"/rest/v1/"+table+"?select="+select+"&order=id", {headers:H});
  return r.json();
}
...
function Spinner(){return <div style={{textAlign:"center",padding:40,color:"#888",fontSize:13}}>Chargement...</div>;}
...
function PilotsForm({pilots,onClose,onRefresh}){
  ...
  async function add(){
    ...
    if(!n||list.find(p=>p.name===n))return;
    setSaving(true);
    const res=await sbInsert("pilots",{name:n,position:list.length});
    if(res&&res[0])setList(l=>[...l,res[0]]);
    setNewName("");setSaving(false);
  }
  async function remove(p){
    setSaving(true);
    await sbDelete("pilots",p.id);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <button style={ss.btnP} onClick={add} disabled={saving}>+ Ajouter</button>
      </div>
...
function ProjForm({data,pilots,onSave,onClose}){
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <div><label style={ss.lbl}>Pilote</label><select style={ss.inp} value={f.pilot||pilots[0]?.name||""} onChange={set("pilot")}>{pilots.map(s=><option key={s.id}>{s.name}</option>)}</select></div>
      ...
      <div style={{gridColumn:"1/-1"}}><label style={ss.lbl}>Avancement : {f.progress||0}%</label><input type="range" min={0} max={100} step={5} value={f.progress||0} onChange={e=>setF(x=>({...x,progress:Number(e.target.value)}))} style={{width:"100%"}}/></div>
...
function GanttView({projects}){
  const active=projects.filter(p=>p.status!=="Terminé"&&p.created_at&&p.deadline);
  if(!active.length)return <p style={{color:"#888",fontSize:13}}>Aucun projet actif avec dates définies.</p>;
  const dates=active.flatMap(p=>[new Date(p.created_at),new Date(p.deadline)]);
  ...
  return(
    <div style={{overflowX:"auto"}}>
      <svg width={W} height={H} style={{fontFamily:"Arial,sans-serif",display:"block"}}>
        ...
        {active.map((p,i)=>{
          const x1=tx(p.created_at),x2=Math.max(tx(p.deadline),x1+6),y=PAD+22+i*ROW,col=GCOLS[i%GCOLS.length],pw=(x2-x1)*(p.progress||0)/100,sn=p.name.length>26?p.name.slice(0,24)+"...":p.name;
          return(<g key={p.id}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead>
          <tr style={{background:"#f4f4f4"}}>
            <th style={{padding:"7px 10px",textAlign:"left",fontWeight:700,color:"#555"}}>Pilote</th>
            ...
            <th style={{padding:"7px 10px",textAlign:"left",fontWeight:700,color:"#555"}}>Charge</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r,i)=>(
...
function PilotCard({pilot,projects,tasks,dateFrom,dateTo}){
  function inRange(item){
    const d=parseD(item.deadline||item.created_at);
    if(!d)return true;
    ...
    <div style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 14px",marginBottom:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
        <div style={{background:"#f0f6ff",borderRadius:7,padding:"8px 10px",textAlign:"center"}}>
          ...
          <div style={{fontSize:20,fontWeight:700,color:"#1a6bbf"}}>{pAll.filter(p=>p.status!=="Terminé").length}</div>
        </div>
        <div style={{background:"#eaf3de",borderRadius:7,padding:"8px 10px",textAlign:"center"}}>
          <div style={{fontSize:10,color:"#555",marginBottom:2}}>Projets terminés</div>
          <div style={{fontSize:20,fontWeight:700,color:"#27500A"}}>{pAll.filter(p=>p.status==="Terminé").length}</div>
        </div>
        <div style={{background:"#f0f6ff",borderRadius:7,padding:"8px 10px",textAlign:"center"}}>
          <div style={{fontSize:10,color:"#555",marginBottom:2}}>Tâches en cours</div>
          <div style={{fontSize:20,fontWeight:700,color:"#1a6bbf"}}>{tAll.filter(t=>t.status!=="Terminé").length}</div>
        </div>
        <div style={{background:"#eaf3de",borderRadius:7,padding:"8px 10px",textAlign:"center"}}>
          <div style={{fontSize:10,color:"#555",marginBottom:2}}>Tâches terminées</div>
          <div style={{fontSize:20,fontWeight:700,color:"#27500A"}}>{tAll.filter(t=>t.status==="Terminé").length}</div>
        </div>
      </div>
      {pAll.length>0&&(
          ...
          {pAll.map(p=>(
            <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 8px",background:p.status==="Terminé"?"#f3fbee":"#f7f9ff",borderRadius:5,marginBottom:3,fontSize:11}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                ...
                <span style={{background:"#e8e8e8",fontSize:9,padding:"1px 5px",borderRadius:3,color:"#555"}}>{p.progress||0}%</span>
              </div>
            </div>
          ))}
        </div>
          ...
          {tAll.map(t=>{
            ...
            return(
              <div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 8px",background:t.status==="Terminé"?"#f3fbee":"#f7f9ff",borderRadius:5,marginBottom:3,fontSize:11}}>
                <div>
                  <span style={{fontWeight:500,color:"#222"}}>{t.name}</span>
                  {prj&&<span style={{color:"#888",marginLeft:6,fontSize:10}}>({prj.name})</span>}
                </div>
                <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
      ...
      {pAll.length===0&&tAll.length===0&&<p style={{fontSize:11,color:"#aaa",margin:0}}>Aucune activité sur cette période.</p>}
    </div>
  );
}
  ...
  return(
    <div>
      <div style={{background:"#f8f8f8",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 14px",marginBottom:16}}>
        <div style={{fontWeight:700,fontSize:13,color:"#111",marginBottom:10}}>Filtres</div>
      ...
      <div>
        {pilotList.map(p=>(
          <PilotCard key={p.id} pilot={p.name} projects={projects} tasks={tasks} dateFrom={dateFrom} dateTo={dateTo}/>
        ))}
      </div>
    </div>
  );
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:12}}>
      <div style={{background:"#fff",borderRadius:12,border:"1px solid #ccc",width:"100%",maxWidth:900,height:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 8px 32px rgba(0,0,0,0.25)"}}>
        ...
        <div style={{background:"#FFF8DC",borderBottom:"1px solid #f0e68c",padding:"7px 16px",fontSize:11,color:"#7a6000",flexShrink:0}}>
          Clique sur <strong>"Imprimer / PDF"</strong> puis Ctrl+P.
        </div>
...
export default function App(){
  ...
  const ET={project_id:projects[0]?.id||1,name:"",status:"En attente",priority:"Moyenne",pilot:pilots[0]?.name||"",site:SITES[0],deadline:"",notes:"",weight:0,created_at:TODAY};

  async function saveP(f){
    const {id,...data}=f;
    if(pModal.mode==="edit"){await sbUpdate("projects",id,data);}
    else{await sbInsert("projects",data);}
    setPModal(null);fetchAll();
  }
  async function saveT(f){
    const {id,...data}=f;
    data.project_id=Number(data.project_id||f.project_id);
    if(tModal.mode==="edit"){await sbUpdate("tasks",id,data);}
    else{await sbInsert("tasks",data);}
    setTModal(null);fetchAll();
  }
  async function delP(id){if(!window.confirm("Supprimer ce projet et ses tâches ?"))return;await sbDelete("projects",id);fetchAll();}
  async function delT(id){await sbDelete("tasks",id);fetchAll();}
  ...
  const avgN=projects.length?Math.round(projects.reduce((s,p)=>s+(p.progress||0),0)/projects.length):0;
  ...
  function buildReport(){
    ...
    const rows=projects.map(p=>{
      const pt=tasks.filter(t=>t.project_id===p.id),odP=isOD(p.deadline,p.status);
      ...
      return'<div style="margin-bottom:13px;border:1px solid #d0d0d0;border-radius:7px;overflow:hidden;page-break-inside:avoid"><div style="background:#f8f8f8;padding:9px 12px;border-bottom:1px solid #d0d0d0"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px">'+sb(p.status)+" "+pb2(p.priority||"Moyenne")+' <span style="font-weight:700;font-size:13px">'+p.name+'</span><span style="font-size:10px;background:#eee;padding:2px 7px;border-radius:4px">'+(p.site||"")+'</span></div>'+(p.description?'<div style="font-size:11px;color:#666;margin-bottom:3px">'+p.description+'</div>':'')+'<div style="font-size:11px;color:#666">Pilote: '+p.pilot+' | Echeance: <span style="'+(odP?"color:#a32d2d;font-weight:700":"")+'">'+(odP?"(!!) ":"")+fd(p.deadline)+'</span> | Avancement: '+(p.progress||0)+'%</div>'+bar(p.progress)+(p.notes?'<div style="margin-top:4px;font-size:11px;color:#555;background:#fffbee;border:1px solid #f0e68c;border-radius:4px;padding:4px 8px">Notes : '+p.notes+'</div>':'')+'</div>'+tr+'</div>';
    }).join("");
    return'<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Bilan Semaine '+wn+'</title><style>body{font-family:Arial,sans-serif;font-size:12px;color:#222;padding:22px 26px;margin:0}.kg{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin-bottom:16px}.kc{background:#f5f5f5;border-radius:6px;padding:8px 10px}.kl{font-size:10px;color:#666;margin:0 0 2px}.kv{font-size:19px;font-weight:700;margin:0}h2{font-size:12px;font-weight:700;border-bottom:1px solid #ddd;padding-bottom:3px;margin:16px 0 9px}@media print{body{padding:0}}</style></head><body><div style="margin-bottom:12px"><div style="font-size:16px;font-weight:700">Bilan hebdomadaire - Physique Medicale</div><div style="font-size:10px;color:#666;margin-top:2px">Sites Galilee &amp; Bourgogne | Semaine '+wn+' | '+dateLabel+'</div></div><div class="kg"><div class="kc"><p class="kl">Projets actifs</p><p class="kv">'+activeN+'</p></div><div class="kc"><p class="kl">Taches en retard</p><p class="kv" style="color:'+(odN>0?"#a32d2d":"#222")+'">'+odN+'</p></div><div class="kc"><p class="kl">Taches terminees</p><p class="kv">'+doneN+'</p></div><div class="kc"><p class="kl">Avancement moyen</p><p class="kv">'+avgN+'%</p></div></div>'+odB+'<h2>Detail des projets</h2>'+rows+'<p style="margin-top:18px;font-size:9px;color:#bbb;text-align:center">Genere le '+dateLabel+' - Confidentiel interne</p></body></html>';
  }
    ...
    <div style={{padding:"1rem 0",fontFamily:"Arial,sans-serif",color:"#111"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"flex-end"}}>
          <button style={ss.btnS} onClick={fetchAll}>↻ Actualiser</button>
      ...
      {view!=="stats"&&<>
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <select style={ss.sel} value={fPil} onChange={e=>setFPil(e.target.value)}>
            <option value="">Tous les pilotes</option>{pilots.map(p=><option key={p.id}>{p.name}</option>)}
          </select>
      ...
      {view==="projects"&&(
        <div>
          ...
          {fp.map(p=>{
            const tc=tasks.filter(t=>t.project_id===p.id).length,od=isOD(p.deadline,p.status);
            ...
            return(
              <div key={p.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"11px 13px",marginBottom:9}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
                  <PBar v={p.progress||0}/><span style={{fontSize:11,color:"#666",minWidth:32}}>{p.progress||0}%</span>
                </div>
            ...
            return(
              <div key={t.id} style={{background:"#fff",border:"1px solid "+(od?"#f09595":"#e0e0e0"),borderRadius:8,padding:"9px 12px",marginBottom:7}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:t.notes?6:0}}>
                  ...
                  <span style={{fontSize:10,color:"#bbb"}}>Ajouté : {fd(t.created_at)}</span>
